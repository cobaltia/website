...
import { NetworthLeaderboard } from "~/types/cobaltia";
import LeaderboardSkeleton from "./leaderboard-skeleton";
import { useInView } from "react-intersection-observer";

export default function NetWorth() {
  const fetchNetworthLeaderboard = async ({
    pageParam,
  }: {
    pageParam: number;
  }) => {
    const data = await apiFetch<NetworthLeaderboard[]>(
      `/leaderboard?id=networth&offset=${pageParam * 10}&limit=${10}`,
    );
    return data;
  };
  const {
    ...
    queryKey: ["leaderboard", "networth"],
    queryFn: fetchNetworthLeaderboard,
    initialPageParam: 0,
